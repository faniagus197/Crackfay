...
│                                                                           crack fb 2.0       │
└──────────────────────────────────────────────────────────────────────────────────────────────┘  	
			{m} ☯︎ {k} ☯︎ {h} ☯︎ {sir} Recode : 🧚FAYZ 🧚 {x}{m} ☯︎ {k} ☯︎ {h} ☯︎ {x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t 🧚: halo kami asisten kamu silakan ikuti perintah dibawah kami akan terhubung jika sukses login  : [green] bot🇨 🇴 🇰 🇮  [white] (;´༎ຶٹ༎ຶ`)'))
		...
		print(f'  {x}[{h}•{x}]{h} Kami cek  LOGIN BERHASIL...!!! {x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s  🧚: KAMI CEK  LOGIN GAGAL....TUMBAL LU MENIGGOY !!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	print('➪ 2. Crack Follower dalam maintamance ')
	print('➪ 3. Crack Grup  dalam maintamance')
	print('➪ 4. Crack File 	')
	print('➪ 5. Hasil Crack  ')
	print('➪ 0. Keluar       ')
